...
func TestNewModelStartsWithRepoSelection(t *testing.T) {
	m := NewModel("owner", []string{"owner/repo1", "owner/repo2"})
	...
	if !strings.Contains(m.View(), "Secrets Audit: Select Repositories") {
		t.Fatalf("expected selector view, got:\n%s", m.View())
	}
}

func TestSelectionConfirmStartsSecretsLoad(t *testing.T) {
	m := NewModel("owner", []string{"owner/repo1", "owner/repo2"})
